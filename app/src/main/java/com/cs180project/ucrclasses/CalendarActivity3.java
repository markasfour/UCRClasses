...
public class CalendarActivity3 extends BaseCalendarActivity {
    ...
    public List<? extends WeekViewEvent> onMonthChange(int newYear, int newMonth) {
        ...
        if(!UCRSchedules.isEmpty(2)) {
//            Log.d("NOTHING", "Not Empty");
            Log.d("NOTHING", "Schedule Size = " + Integer.toString(UCRSchedules.getSize(2)));
            for (int i = 0; i < UCRSchedules.getSize(2); i++) {
                for (int j = 0; j < UCRSchedules.getDays(2, i).length(); j++) {
                    startTime = Calendar.getInstance();
                    startTime.set(Calendar.HOUR_OF_DAY, UCRSchedules.getStartHour(2, i));
                    startTime.set(Calendar.MINUTE, UCRSchedules.getStartMin(2, i));
...
//                    Log.d("NOTHING", "Minute start = " + Integer.toString(UCRSchedules.getStartMin(0, i)));
                    ...
                    Log.d("NOTHING", UCRSchedules.getDays(2, i));
                    if (UCRSchedules.getDays(2, i).charAt(j) == 'M')
                        startTime.set(Calendar.DAY_OF_WEEK, Calendar.MONDAY);
                    else if (UCRSchedules.getDays(2, i).charAt(j) == 'T')
                        startTime.set(Calendar.DAY_OF_WEEK, Calendar.TUESDAY);
                    else if (UCRSchedules.getDays(2, i).charAt(j) == 'W')
                        startTime.set(Calendar.DAY_OF_WEEK, Calendar.WEDNESDAY);
                    else if (UCRSchedules.getDays(2, i).charAt(j) == 'R')
                        startTime.set(Calendar.DAY_OF_WEEK, Calendar.THURSDAY);
                    else if (UCRSchedules.getDays(2, i).charAt(j) == 'F')
                        startTime.set(Calendar.DAY_OF_WEEK, Calendar.FRIDAY);
                    else {
                        Log.d("ERROR", "Invalid day: " + UCRSchedules.getDays(2, i).charAt(j) + " for call number: " + UCRSchedules.getCallNum(2, i));
                        startTime.set(Calendar.DAY_OF_WEEK, Calendar.SATURDAY);
                    }

                    endTime = (Calendar) startTime.clone();
                    endTime.add(Calendar.HOUR_OF_DAY, (UCRSchedules.getEndHour(2, i) - UCRSchedules.getStartHour(2, i)) );
                    endTime.add(Calendar.MINUTE, UCRSchedules.getEndMin(2, i) - UCRSchedules.getStartMin(2, i));
...
//                    Log.d("END MIN", Integer.toString(UCRSchedules.getEndMin(0, i) - UCRSchedules.getStartMin(0, i)));
                    ...
                    Log.d("NOTHING", UCRSchedules.getCourseNum(2, i) +" "+ UCRSchedules.getCourseType(2, i));
                    event = new WeekViewEvent(Integer.parseInt(UCRSchedules.getCallNum(2, i)), UCRSchedules.getCourseNum(2, i) + " " + UCRSchedules.getCourseType(2, i), startTime, endTime);
                    switch (i%4){
                        case 0:
